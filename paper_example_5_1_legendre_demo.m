function paper_example_5_1_legendre_demo()
% Paper Example 5.1 on the reference square [-1,1]^2.
%
% Physical domain: (x,y) in (0,1)^2 with x = (xi+1)/2, y = (eta+1)/2.
% Exact Monge-Ampere solution:
%   u(x,y) = exp((x^2 + y^2)/2)
% Source term:
...
% with boundary lifting Ub built from the Dirichlet data g = u|_{dOmega}.
    ...
    output_dir = fullfile(pwd, 'output', 'paper_example_5_1');
    plot_example(problem, result, output_dir);

    fprintf('Example 5.1 finished with N = %d\n', problem.N);
...
function problem = default_problem()
    ...
    problem.u_exact = @(x, y) exp((x.^2 + y.^2) / 2);
...
function data = build_example_data(problem, ops)
    ...
    Ub_q = boundary_extension_example_5_1(xq, yq, problem.u_exact);
    Ub_eval = boundary_extension_example_5_1(xe, ye, problem.u_exact);
...
function vals = boundary_extension_example_5_1(x, y, ufun)
    g0y = ufun(0 * y, y);
    g1y = ufun(1 + 0 * y, y);
    gx0 = ufun(x, 0 * x);
    gx1 = ufun(x, 1 + 0 * x);
...
function plot_example(problem, result, output_dir)
    ...
    exportgraphics(fig1, fullfile(output_dir, 'example_5_1_surfaces.png'), 'Resolution', 180);
    close(fig1);

    fig2 = figure('Visible', 'off', 'Color', 'w', 'Position', [120, 120, 700, 520]);
    semilogx(result.eps_values, result.l2_error, '-o', 'LineWidth', 1.6, 'MarkerSize', 7);
    hold on;
    semilogx(result.eps_values, result.h1_error, '-s', 'LineWidth', 1.6, 'MarkerSize', 7);
    semilogx(result.eps_values, result.h2_error, '-d', 'LineWidth', 1.6, 'MarkerSize', 7);
    set(gca, 'XDir', 'reverse');
    grid on;
    xlabel('\epsilon');
    ylabel('error');
    title(sprintf('Example 5.1 Error Curves, N = %d', problem.N));
    legend('L2', 'H1', 'H2', 'Location', 'southwest');
    exportgraphics(fig2, fullfile(output_dir, 'example_5_1_error_curves.png'), 'Resolution', 180);
    close(fig2);
end
