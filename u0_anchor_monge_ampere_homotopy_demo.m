function output = u0_anchor_monge_ampere_homotopy_demo(case_name, options)
% Legendre-Galerkin continuation for the anchored homotopy
%
...
%   det(D^2 u) = f.
%
% We keep the paper's Legendre-Galerkin basis and B-matrix transform chain,
% but replace the paper's vanishing-moment regularization by the anchored
% mass-type homotopy term.
%
...
function sweep_result = run_n_convergence_sweep(base_problem, options)
    ...
    [p_values, use_paired_targets] = parse_n_sweep_p_values(options, N_values);
...
function [p_values, use_paired_targets] = parse_n_sweep_p_values(options, N_values)
...
function result = solve_t_homotopy(problem)
    ...
    while current_t > problem.t_end && accepted < num_slots && isempty(failure_reason)
        ...
        if abs(target_t - problem.t_end) <= 10 * eps(max(1, abs(current_t)))
            target_t = problem.t_end;
        end
